...
public class DummyGame implements IGameLogic {
    ...
    public DummyGame(String filename) {
    ...
    public void init(Window window) throws Exception {
        ...
        // Load the triangle mesh from an input obj file
        Mesh mesh = OBJLoader.loadMesh(inputFilename);
        Material material = new Material(new Vector3f(0.2f, 0.5f, 0.5f), reflectance);
        mesh.setMaterial(material);

        // Convert the mesh into an HalfEdgeMesh
        halfEdgeMesh = new HalfEdgeMesh(mesh);
        //mesh = halfEdgeMesh.toMesh();
        //mesh.setMaterial(material);
        
        GameItem gameItem = new GameItem(mesh);
        gameItem.setScale(0.5f);
        gameItem.setPosition(-0.5f, -0.5f, -2);
        gameItem.setRotation(177.4f, 213.6f, 0);
        ...
        glfwSetKeyCallback(window.getWindowHandle(), new GLFWKeyCallback() {
            ...
            public void invoke(long window, int key, int scancode, int action, int mods) {
                if (key == GLFW_KEY_ESCAPE && action == GLFW_RELEASE) {
                    ...
                    gameItems[currentObj].setPosition(-0.5f, -0.5f, -2);
                    gameItems[currentObj].setRotation(177.4f, 213.6f, 0f);
                    ...
                    gameItems[currentObj].setScale(0.5f);
                    gameItems[currentObj].setPosition(-0.5f, -0.5f, -2);
                    gameItems[currentObj].setRotation(177.4f, 213.6f, 0);
                }
            }
        });
    ...
    public void update(float interval, MouseInput mouseInput) {
        ...
        if (mouseInput.isLeftButtonPressed()) {
            ...
            System.out.println(gameItems[0].getRotation());
        }
